...
- [ ] 1.1 Introduce new domain packages under `src/fsspeckit/`:
...
- [ ] 1.2 Create `fsspeckit.core.filesystem` as the canonical home for
...
- [ ] 1.3 Refactor `fsspeckit.utils` into a thin façade that re-exports from
  `datasets`, `sql`, and `common` modules, keeping the existing public names
  available for backwards compatibility.

## 2. Code moves and aliases

- [ ] 2.1 Move or alias DuckDB dataset helpers from
  ...
  new package.
- [ ] 2.2 Move or alias PyArrow dataset helpers from
...
- [ ] 2.3 Move or alias SQL filter helpers from `fsspeckit.utils.sql` into
  `fsspeckit.sql.filters`, keeping the existing function names and signatures
  intact.
- [ ] 2.4 Move or alias cross-cutting utilities (`datetime`, `logging`,
...
- [ ] 3.1 Add import rules (and, where practical, static checks) to ensure
  that:
...
- [ ] 3.2 Update type-checking and linting configuration (if needed) so that
  new packages are included in `mypy` and `ruff` runs.

## 4. Public API and docs

- [ ] 4.1 Update documentation and examples to use the new domain packages as
...
- [ ] 4.2 Update API reference configuration (MkDocs/mkdocstrings) to include
  `fsspeckit.datasets`, `fsspeckit.sql`, and `fsspeckit.common` sections.
- [ ] 4.3 Add a short migration guide explaining the new layout and mapping
  from old imports (`fsspeckit.utils.*`) to the new domain packages.

## 5. Validation

- [ ] 5.1 Run `openspec validate refactor-package-layout-fsspeckit --strict`
  and ensure the change passes.
- [ ] 5.2 Run the full test suite (`uv run pytest`) and the documentation
