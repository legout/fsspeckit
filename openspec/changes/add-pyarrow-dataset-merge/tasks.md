...
- [ ] 1.4 Implement merge execution using `pyarrow.dataset` and
        `pyarrow.compute`:
...
- [ ] 1.5 Implement deduplication logic for the `deduplicate` strategy by
...
- [ ] 1.6 Implement merge statistics calculation so that the returned dict
        contains `inserted`, `updated`, `deleted`, `total` and matches the
        DuckDB helper’s interpretation.
- [ ] 1.7 Implement writing of the merged dataset back to `target_path` using
        `pyarrow.parquet` and an fsspec filesystem, with a configurable
        `compression` codec.

## 2. Testing

- [ ] 2.1 Add unit tests to `tests/test_utils/test_utils_pyarrow.py` (or a new
...
- [ ] 2.2 Add tests that simulate larger targets by many small files and verify
        that:
...
- [ ] 3.1 Add a “PyArrow Parquet Merge” section to `docs/utils.md`:
...
- [ ] 3.2 Update any relevant API docs (e.g.
