...
- [ ] 1.2 Verify correct handling of:
        ...
        - datetime/date/time literals for columns with timestamp/date/time
          PyArrow types.
- [ ] 1.3 Ensure the implementation does not rely on DuckDB-specific behavior
        and can be used directly with PyArrow datasets for row-level pruning.

## 2. Testing

- [ ] 2.1 Treat `tests/test_utils/test_utils_sql.py` as contract tests for
        ...
        - Edge cases around booleans and time-based columns under current
          PyArrow versions.
- [ ] 2.2 Add a small dataset-level test that:
        - Builds a `pyarrow.dataset.Dataset` from a temp parquet directory.
        - Uses `sql2pyarrow_filter` to construct a filter expression.
        - Applies the filter via `scanner.to_table(filter=expr)` and asserts
          that the filtered rows match expectations.

## 3. Documentation

- [ ] 3.1 Add a short section in `docs/utils.md` or a dedicated SQL utilities
        doc describing `sql2pyarrow_filter`, its supported SQL subset, and how
        to use it with `pyarrow.dataset`.

## 4. Validation

- [ ] 4.1 Run `pytest tests/test_utils/test_utils_sql.py`.
- [ ] 4.2 Run `ruff check src/fsspeckit/utils/sql.py`.
- [ ] 4.3 Run `mypy --ignore-missing-imports src/fsspeckit/utils/sql.py`.
