...
class TestAtomicLocalOrderedCompactionInvariants:
    def test_partition_placement_preserved(self, tmp_path):
        ...
        _write_partitioned(
            ...
            {
                "region=US": [
                    ("a.parquet", pa.table({"id": [3, 1], "region": ["US", "US"]}))
                ],
                "region=DE": [
                    ("a.parquet", pa.table({"id": [9, 1], "region": ["DE", "DE"]}))
                ],
            },
        )
        result, _ = _execute(dataset, ["+id"], target_rows_per_file=10)
    ...
    def test_row_count_preserved(self, tmp_path):
        ...
        total = sum(
            pq.read_metadata(p).num_rows for p in _partition_files(str(dataset), "")
        )
        assert total == 5
...
class TestAtomicLocalOrderedCompactionPartitionFilter:
    def test_filter_restricts_scope_and_keeps_partition_order(self, tmp_path):
        ...
        _write_partitioned(
            ...
            {
                "region=US": [
                    ("a.parquet", pa.table({"id": [3, 1, 2], "region": ["US"] * 3}))
                ],
                "region=DE": [
                    ("a.parquet", pa.table({"id": [9, 1, 8], "region": ["DE"] * 3}))
                ],
            },
        )
...
class TestAtomicLocalOrderedCompactionSafety:
    def test_externally_constructed_plan_rejects_full_distinct_key_scan(self, tmp_path):
        dataset = tmp_path / "ds"
        _write_partitioned(
            str(dataset),
            {"": [("a.parquet", pa.table({"id": [1, 2]}))]},
...
class TestAtomicLocalOrderedCompactionMemorySpill:
    ...
    def test_budget_zero_forces_external_merge_sort_with_spill_dir(self, tmp_path):
        dataset = tmp_path / "ds"
        ...
        if os.path.isdir(str(spill_dir)):
            assert not any(f.endswith(".parquet") for f in os.listdir(str(spill_dir)))

    def test_spill_across_multiple_source_files_preserves_global_order(self, tmp_path):
        dataset = tmp_path / "ds"
        spill_dir = tmp_path / "spill"
    ...
    def test_budget_zero_without_spill_directory_rejected_at_plan_time(self, tmp_path):
        dataset = tmp_path / "ds"
    ...
    def test_default_budget_rejection_when_partition_exceeds_it(
        ...
        monkeypatch.setattr(mod, "_DEFAULT_ORDERED_COMPACTION_MEMORY_BUDGET_MB", 0)
