...
## Dataset write helpers (removed)

`write_pyarrow_dataset()` and the legacy convenience helpers (`insert_dataset`, `upsert_dataset`, `update_dataset`, `deduplicate_dataset`) were removed.
...
merge = io.merge(table, "dataset/", strategy="upsert", key_columns=["id"])
```

See `docs/dataset-handlers.md` for the canonical dataset API.
